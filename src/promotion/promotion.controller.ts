...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const promotions = await promotionService.findAll(activeOnly);
    res.json(promotions);
  } catch (error) {
    next(error);
  }
...
router.patch('/:code', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const promotion = await promotionService.update(req.params.code, data);
    res.json(promotion);
  } catch (error) {
    next(error);
  }
...
router.delete('/:code', async (req: Request, res: Response, next: NextFunction) => {
  try {
    await promotionService.delete(req.params.code);
    res.status(204).send();
  } catch (error) {
    next(error);
  }
